 ...
 * Ссылка чипа.
 *
 * Порядок id в `sub` — это приоритет подкатегорий, поэтому только что
 * выбранная встаёт ПЕРВОЙ: её товары показываются выше товаров выбранных
 * ранее (см. getVisibleGroups). Повторный клик по выбранному чипу снимает
 * выбор — остальные при этом сохраняют свой относительный порядок, а чтобы
 * поднять подкатегорию наверх, её выбирают заново.
 */
function buildHref(
  categorySlug: string,
  selectedIds: string[],
 ...
 * порядок = порядок выбора пользователем, от последней выбранной к самой
 * ранней — именно в этом порядке товары потом группируются на странице,
 * см. getVisibleGroups и CatalogProducts).
 *
