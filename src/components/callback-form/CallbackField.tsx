 ...
 */
export const CallbackField = forwardRef<HTMLInputElement | HTMLTextAreaElement, CallbackFieldProps>(
  function CallbackField({ label, name, error, required, hint, as = 'input', className, ...rest }, ref) {
    ...
    const sharedClassName = [
      ...
      error ? 'border-[var(--danger)]' : 'border-[var(--border)] hover:border-[var(--text-muted)]',
      className ?? '',
    ].join(' ')
    ...
    return (
      <div className="flex flex-col gap-1.5">
        ...
        {error ? (
          ...
          <p id={`${fieldId}-hint`} className="text-[13px] text-[var(--text-muted)]">
            {hint}
          </p>
        ) : null}
      </div>
