...
const ProductInfo: React.FC<ProductInfoProps> = ({ product }) => {
  ...
  return (
    <div id="product-info" className="flex flex-col gap-5">
      {/* Название */}
      <h1 className="text-[clamp(1.5rem,3vw,2rem)] font-bold leading-tight text-text-primary">
        {product.name}
      </h1>

      {/* Описание */}
      {product.description && (
        <p className="text-[0.9375rem] leading-relaxed text-text-secondary whitespace-pre-line">
          {product.description}
        </p>
      )}
